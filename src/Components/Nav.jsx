...
function Nav({ isDarkMode }) {
  const navClasses = `border-transparent border-x-4 hover:border-violet-300 hover:border-solid p-2 sm:p-4 -mb-1 bg-opacity-90
  ${isDarkMode ? 'hover:bg-gray-800' : 'hover:bg-violet-200'}`;
  return (
    <div
      className={`pt-2 fixed top-0 left-0 right-0 z-50 h1 text-lg lg:text-3xl uppercase border-b-4 drop-shadow-2xl bg-opacity-90 ${
        isDarkMode ? 'text-violet-300 bg-gray-800' : 'text-violet-900 bg-violet-200'
      } border-violet-300`}
    >
